  ...
  try {
    ...
    const [resResult, itemsResult, paymentsResult] = await Promise.all([
      pool.query(`
        ...
        WHERE r.id = $1
      `, [reservationId]),
      pool.query(`SELECT id, reservation_id, charge_type, description, quantity, unit_price, amount, voided, void_reason, TO_CHAR(posted_at AT TIME ZONE 'Asia/Manila', 'YYYY-MM-DD"T"HH24:MI:SS"+08:00"') as posted_at FROM hotel_folio_items WHERE reservation_id = $1 ORDER BY posted_at ASC`, [reservationId]),
      pool.query(`SELECT id, reservation_id, payment_method, amount, reference, voided, notes, cashier_name, TO_CHAR(posted_at AT TIME ZONE 'Asia/Manila', 'YYYY-MM-DD"T"HH24:MI:SS"+08:00"') as posted_at FROM hotel_folio_payments WHERE reservation_id = $1 ORDER BY posted_at ASC`, [reservationId]),
    ]);
  ...
  try {
    ...
    let query = `
      SELECT p.id, p.reservation_id, p.payment_method, p.amount, p.reference, p.voided, p.notes, p.cashier_name,
             TO_CHAR(p.posted_at AT TIME ZONE 'Asia/Manila', 'YYYY-MM-DD"T"HH24:MI:SS"+08:00"') as posted_at,
    ...
    let discountQuery = `
      SELECT i.id, i.reservation_id, i.charge_type, i.description, i.quantity, i.unit_price, i.amount, i.voided,
             TO_CHAR(i.posted_at AT TIME ZONE 'Asia/Manila', 'YYYY-MM-DD"T"HH24:MI:SS"+08:00"') as posted_at,
             r.full_name as guest_name, r.room_number

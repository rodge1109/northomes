  ...
  try {
    ...
    const [resResult, itemsResult, paymentsResult] = await Promise.all([
      pool.query(`
        ...
        WHERE r.id = $1
      `, [reservationId]),
      pool.query(`SELECT id, reservation_id, charge_type, description, quantity, unit_price, amount, voided, void_reason, TO_CHAR(posted_at AT TIME ZONE 'Asia/Manila', 'YYYY-MM-DD HH24:MI:SS') as posted_at FROM hotel_folio_items WHERE reservation_id = $1 ORDER BY posted_at ASC`, [reservationId]),
      pool.query(`SELECT id, reservation_id, payment_method, amount, reference, voided, notes, cashier_name, TO_CHAR(posted_at AT TIME ZONE 'Asia/Manila', 'YYYY-MM-DD HH24:MI:SS') as posted_at FROM hotel_folio_payments WHERE reservation_id = $1 ORDER BY posted_at ASC`, [reservationId]),
    ]);

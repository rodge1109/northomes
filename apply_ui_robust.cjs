...
const startIdx = lines.findIndex(l => l.includes('{/* Message View */}'));
if (startIdx !== -1) {
  let endIdx = startIdx;
  while (endIdx < lines.length && !lines[endIdx].includes('// Header Component')) {
    endIdx++;
  }
  
  if (endIdx > startIdx) {
    const newSection = `        {/* Message View */}
    ...
    // endIdx points to "// Header Component"
    ...
    // 5160: }
    // 5161: 
    // 5162: // Header Component
    
    // So we want to replace from startIdx to endIdx - 6
    const numLinesToRemove = (endIdx - 6) - startIdx + 1;
    lines.splice(startIdx, numLinesToRemove, newSection);
    fs.writeFileSync('src/App.jsx', lines.join('\\n'));
    console.log('Replaced successfully using robust indices.');
  }
}
